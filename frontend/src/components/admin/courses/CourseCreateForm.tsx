...
export function CourseCreateForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      const course = await apiClient.post<CourseDetail, FormData>(
        "/api/courses/courses/",
        form,
      );
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <Input id="course_title" name="title" required />
      </div>
      <div className="space-y-2">
        <Label htmlFor="course_description">Description</Label>
        <Textarea id="course_description" name="description" rows={5} />
      </div>
      <div className="space-y-2">
      ...
      <div className="space-y-2">
        <Label htmlFor="course_thumbnail">Thumbnail (optional)</Label>
        <Input
          id="course_thumbnail"
          name="thumbnail"
          type="file"
      ...
      <label className="flex items-center gap-2 text-sm">
        <input type="checkbox" name="is_published" className="size-4" /> Publish
        immediately
      </label>
