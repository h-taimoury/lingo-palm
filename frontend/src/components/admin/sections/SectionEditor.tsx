...
export function SectionEditor({ section }: { section: SectionDetail }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      await apiClient.patch(`/api/courses/sections/${section.id}/`, body);
      router.refresh();
    } catch (caught) {
      setError(caught);
    } finally {
  ...
  return (
    <form onSubmit={submit} className="space-y-4 rounded-xl border bg-card p-5">
      ...
      <div className="space-y-2">
        <Label htmlFor="section_video">Video URL</Label>
        <Input
          id="section_video"
          name="video_url"
          type="url"
        ...
        <p className="text-xs text-muted-foreground">
          Progressive files and .m3u8 HLS streams are supported by the lesson
          player.
        </p>
      </div>
