...
export function WordDetails({ entryId, currentSenseId }: { entryId: number; currentSenseId: number }) {
  ...
  useEffect(() => {
    ...
    void apiClient.get<Entry[]>(`/api/dictionary/entries/${entryId}/full-word/?schema=level-v1`, {
      ...
      cache: "default", // Respect the endpoint's 24-hour browser cache lifetime.
    })
        ...
        <section key={entry.id} className="border-t pt-5 first:border-0 first:pt-0" aria-label={`${entry.word}, ${entry.part_of_speech}`}>
          <div className="space-y-6">
            {entry.senses.map((sense, index) => (
              <div key={sense.id}>
                <SenseView sense={{ ...sense, entry }} learned={false} examplesMode="all" showHeader={index === 0} keyboardShortcuts={false} showMetadata currentSense={sense.id === currentSenseId} frequencyLabels={entry.frequency} />
              </div>
            ))}
