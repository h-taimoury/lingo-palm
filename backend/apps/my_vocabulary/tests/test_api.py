...
class VocabularyBulkActionApiTests(APITestCase):
    ...
    def test_unset_already_known_only_updates_existing_rows(self):
        ...
        response = self._bulk(
            {
                "action": "unset_already_known",
                "sense_ids": [self.sense_1.id, self.sense_2.id],
            }
        )
        self.assertEqual(response.status_code, 200, response.data)
        self.assertFalse(
    ...
    def test_set_needs_review_and_unset_needs_review(self):
        ...
        response = self._bulk(
            {"action": "set_needs_review", "sense_ids": [self.sense_1.id]}
        )
        self.assertEqual(response.status_code, 200, response.data)
        self.assertTrue(
        ...
        response = self._bulk(
            {"action": "unset_needs_review", "sense_ids": [self.sense_1.id]}
        )
        self.assertEqual(response.status_code, 200, response.data)
        self.assertFalse(
            Vocabulary.objects.get(user=self.user, sense=self.sense_1).needs_review
    ...
    def test_unset_learned_deletes_rows_and_reports_deleted_sense_ids(self):
        ...
        self.assertEqual(response.status_code, 200, response.data)
        self.assertEqual(response.data["sense_ids"], [self.sense_1.id])
    ...
    def test_list_filters_by_needs_review(self):
        ...
        Vocabulary.objects.create(
            user=self.user, sense=self.sense_2, needs_review=False
        )
